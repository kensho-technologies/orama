...
export default React.createClass({
  ...
  render() {
    ...
    const xDomain = utils.rect.getDomainX(plotRect)
    const xRange = this.props.xRange || d3Arrays.extent(data, R.prop(xProp))
    const xScale = d3Scale.linear()
      .range(xRange)
      .domain(xDomain)
      .nice(utils.ticks.getYCount(xDomain))
    const xMap = R.pipe(R.prop(xProp), xScale)
    const yDomain = utils.rect.getDomainY(plotRect)
    const yRange = this.props.yRange || d3Arrays.extent(data, R.prop(yProp))
